...
public class XmlMovieReader extends AbstractMetadataResourceMovieReader implements MovieReader {
    ...
    public List<Movie> loadMovies() {
        try {
            // InputStream content = ClassLoader.getSystemResourceAsStream(getMetadata());
            final InputStream content = getMetadataResource().getInputStream();
            final Source source = new StreamSource(content);

            MovieMetadata metadata = (MovieMetadata) unmarshaller.unmarshal(source);
    ...
    public static class MovieData {
        ...
        public Movie toMovie() {
            return Movie.of(this.title, this.genres, this.language, this.country, this.releaseYear, this.director, this.actors, this.imdbLink, this.watchedDate);
        }
    }
}
